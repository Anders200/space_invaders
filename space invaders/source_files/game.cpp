...
{   
    ...
    {
        lifes* l = new lifes();
        l->setPosition(800.0f + i * 60.0f, 20.0f);
        playerLifes.push_back(l);
    }
}
...
void Game::checkForPlayerCollision()
{
    for (int i = 0; i < enemyBullets.size(); i++)
    {
        ...
        if (b->getGlobalBounds().intersects(player.getGlobalBounds()))
        {
            ...
            enemyBullets.erase(std::remove(enemyBullets.begin(), enemyBullets.end(), b), enemyBullets.end());
            // Game over
            if (playerLifes.size() == 0)
            {
                this->window->close();
            }
            else
            {
                delete playerLifes.back();
                playerLifes.pop_back();
            }
        }
    }
}
...
void Game::render() // renders game objects
{
    ...
    for (int i = 0; i < playerLifes.size(); i++)
    {
        this->window->draw(*playerLifes.at(i));
    }
    this->window->display();
}
